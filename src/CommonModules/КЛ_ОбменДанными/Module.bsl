...
Функция ОбработатьВходящийPOSTЗапрос(Запрос) Экспорт
	...
	Для Каждого Элемент Из ДанныеЗапроса Цикл
		...
		Если ЗначениеЗаполнено(Элемент.Фото) Тогда
			ДвоичныеДанныеФото = Base64Значение(Элемент.Фото);
			ДоставкаОбъект.Фотография = Новый ХранилищеЗначения(ДвоичныеДанныеФото);
		КонецЕсли;
		//Заполняем статус
		Если ЗначениеЗаполнено(Элемент.Статус) Тогда
			ДоставкаОбъект.Статус = Перечисления.КЛ_СтатусыДоставок[Элемент.Статус]
		Иначе
			ДоставкаОбъект.Статус = Перечисления.КЛ_СтатусыДоставок.ПустаяСсылка();
		КонецЕсли;

		ДоставкаОбъект.Комментарий = Элемент.Комментарий;
